...
class BaseUtil():
    ...
    def get_field_names_gen(cls, stop_list = None):
        return_list = []
        if not isinstance(stop_list, collections.Iterable):
            stop_list = ['id',]
        for f in cls._meta.get_fields():
            if f.name in stop_list:
                continue
...
MODEL_NAME_GEN = ((item.Other.url,item._meta.verbose_name_plural) for item in MODEL_LIST)
MODEL_DICT = {item._meta.model_name:item for item in MODEL_LIST}